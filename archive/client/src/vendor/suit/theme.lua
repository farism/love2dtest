...
function theme.Label(text, opt, x, y, w, h)
  ...
  love.graphics.setColor(
    (opt.color and opt.color.normal or {}).fg or theme.color.normal.fg
  )
...
function theme.Slider(fraction, opt, x, y, w, h)
  ...
  if opt.state ~= nil and opt.state ~= 'normal' then
    love.graphics.setColor(
      (opt.color and opt.color.active or {}).fg or theme.color.active.fg
    )
    if opt.vertical then
...
function theme.Input(input, opt, x, y, w, h)
  ...
  theme.drawBox(
    ...
    w,
    h,
    (opt.color and opt.color.normal) or theme.color.normal,
    opt.cornerRadius
  )
  x = x + 3
  w = w - 6
  ...
  -- text
  love.graphics.setColor(
    (opt.color and opt.color.normal and opt.color.normal.fg) or
      theme.color.normal.fg
  )
  love.graphics.setFont(opt.font)
