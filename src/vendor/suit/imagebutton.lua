...
return function(core, normal, ...)
  ...
  core:registerMouseHit(
    ...
    function(u, v)
      ...
      if opt.mask then
        -- alpha test
        assert(
          isType(opt.mask, 'ImageData'),
          'Option `mask` is not a love.image.ImageData'
        )
        assert(
          u < mask:getWidth() and v < mask:getHeight(),
          'Mask may not be smaller than image.'
        )
  ...
  core:registerDraw(
    opt.draw or function(image, x, y, r, g, b, a)
