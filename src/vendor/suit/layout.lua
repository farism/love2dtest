...
function Layout:pop()
  ...
  self._x,
    ...
    self._h,
    self._widths,
    self._heights = unpack(self._stack[#self._stack])
...
local function layout_retained_mode(
  self,
  t,
  constructor,
  string_argument_to_table,
  fill_width,
  fill_height)
  -- sanity check
  ...
  if type(p) ~= 'table' then
    error(
      "Invalid argument `padding' (table expected, got " .. type(p) .. ')',
      2
    )
  end
  ...
  local fill_h =
    fill_height(layout, t.min_height or 0, self._y + self._h - p[2])
  local dx, dy = 0, 0
